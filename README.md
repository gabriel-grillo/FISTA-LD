...
- `code/`
    ...
    - `paper_results_data_reference/`: Reference data generated by `paper_results.py`.
    - `data/mayo_clinic_image/`: Directory where the raw data should be located.
...
```
docker run --rm --runtime=nvidia --gpus '"device=0"' --ipc=host --mount type=bind,source=$(pwd)/code,target=/code -it fista-ld
```
